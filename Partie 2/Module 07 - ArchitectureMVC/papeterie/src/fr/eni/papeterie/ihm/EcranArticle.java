...
public class EcranArticle extends JFrame implements IPanelBoutonObserver  {
	...
	public PanelBoutons getPanelBoutons() {
		if (panelBoutons == null) {
			...
			panelBoutons.addPanelBoutonObserver(this);
	...
	public Article getArticle() {
		...
		if(getRadioStylo().isSelected()){
			article = new Stylo(idCourant, getTxtMarque().getText(), getTxtReference().getText(), getTxtDesignation().getText(), Float.parseFloat(getTxtPrix().getText()), Integer.parseInt(getTxtStock().getText()), (String) getCboCouleur().getSelectedItem());
		}else{
			article = new Ramette(idCourant, getTxtMarque().getText(), getTxtReference().getText(), getTxtDesignation().getText(), Float.parseFloat(getTxtPrix().getText()), Integer.parseInt(getTxtStock().getText()), getChk80().isSelected()?80:100);
		}
